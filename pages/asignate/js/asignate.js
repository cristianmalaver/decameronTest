...
function getDataObligation(table, dataSetObligation, typeSend) {
    try {
        loadPageView();
        var xhttp = new XMLHttpRequest();
        var arrayCell = new Array("Nombre Hotel", "cantidad","Tipo de habitacion", "acomodacion","Acciones");
        var JsonData;
                ...
                if (xhttp.responseText != "") {
                    ...
                    if (jsonObj.length != 0) {
                        ...
                        if (typeSend == 2 || typeSend == 1) {
                            ...
                            tableObligation.createTableHotels();
                            console.log(jsonObj);
                        } else if (typeSend == 0) {
                            //console.log(jsonObj);
                            //setDataForm(jsonObj);
...
function loadView() {
    getListHotels("id_create_product");
    loadPageView();
    getDataObligation("tableObligation", "", 2);
    getActionStorage();
}
//**Function get List Bank **/
function getListHotels(idSelect) {
    try {
        var xhttp = new XMLHttpRequest();
        var JsonData;
        xhttp.open("POST", "../../php/bo/bo_obligation.php", true);
